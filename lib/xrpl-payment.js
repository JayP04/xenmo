...
export async function getRate(fromCurrency, toCurrency, amount, senderAddress, receiverAddress) {
  ...
  return {
    ...
    rateSource,
    competitors: {
      westernUnion: { fee: 12, fxMarkup: 0.04, total: (parseFloat(amount) * 0.04 + 12).toFixed(2) },
      wise: { fee: 1.50, fxMarkup: 0.005, total: (parseFloat(amount) * 0.005 + 1.50).toFixed(2) },
      bankWire: { fee: 35, fxMarkup: 0.03, total: (parseFloat(amount) * 0.03 + 35).toFixed(2) },
      remitx: { fee: 0, fxMarkup: 0, total: feeInSender.toFixed(2) },
    },
  };
}
