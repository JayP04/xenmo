...
// Burn tokens from a user by sending them back to the issuer.
// Called when creating an escrow so the sender's token balance decreases.
export async function burnTokens(senderSeed, currency, amount) {
  const client = await getClient();
  const issuer = config.issuers[currency];
  if (!issuer) throw new Error(`No issuer configured for ${currency}`);
...
// Mint tokens to the claimer from the issuer.
// Called when claiming an escrow so the claimer's token balance increases.
export async function mintTokensForClaim(claimerAddress, currency, amount) {
  const client = await getClient();
  const issuer = config.issuers[currency];
  if (!issuer) throw new Error(`No issuer configured for ${currency}`);
  ...
  const result = await client.submitAndWait(
    {
      ...
      Destination: claimerAddress,
      Amount: { currency, issuer: issuerWallet.address, value: String(amount) },
    },
    { wallet: issuerWallet }
  );
